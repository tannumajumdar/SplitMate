...
export default function Header() {
    ...
    <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 z-50 overflow-hidden animate-scale-in">
      <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
        <span className="text-sm font-semibold text-slate-900 dark:text-white">
          Notifications{' '}
          {unreadCount > 0 && (
            <span className="ml-1 text-xs bg-rose-100 text-rose-600 px-1.5 py-0.5 rounded-full">{unreadCount}</span>
          )}
        </span>
        {unreadCount > 0 && (
          <button
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/80 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 h-16 flex items-center px-4 sm:px-6">

      {/* ── Mobile header: 3-column grid ─────────────────────────────────── */}
      <div className="lg:hidden w-full grid grid-cols-3 items-center">
        ...
        <div className="flex items-center">
          ...
          >
            <IoSparkles size={18} color="white" />
          </div>
        </div>

        {/* Center: page title */}
        <div className="flex items-center justify-center">
          <h2 className="text-[15px] font-bold text-slate-900 dark:text-white tracking-tight truncate">{title}</h2>
        </div>
        ...
        <div className="flex items-center justify-end gap-0.5">
          ...
          <div className="relative" ref={notifRef}>
            ...
            >
              {notifOpen ? <IoNotifications size={17} /> : <IoNotificationsOutline size={17} />}
              {unreadCount > 0 && (
          ...
          {/* Avatar */}
          {user ? (
            <button onClick={() => navigate('/profile')} className="ml-0.5">
              <Avatar name={user.name} color={user.avatarColor} size="sm" ring />
            </button>
          ) : (
            <button
              onClick={() => navigate('/login')}
              className="ml-0.5 w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold text-white"
      ...
      {/* ── Desktop header: left title + right actions ───────────────────── */}
